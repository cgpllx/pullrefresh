...
import com.kubeiwu.pull.pulltorefresh.listview.KPullFreshFooter;
import com.kubeiwu.pull.pulltorefresh.listview.KPullFreshHeader;
...
public class KGridView extends HeaderFooterGridView implements OnScrollListener {
	...
	// -- header view
	private KPullFreshHeader mHeaderView;
	...
	// -- footer view
	private KPullFreshFooter mFooterView;
	...
	private void initWithContext(Context context, KConfig config) {
		...
		// init header view
		mHeaderView = new KPullFreshHeader(context, config);
...
//		mHeaderView.setVisiableHeight(100);
		// init footer view
		mFooterView = new KPullFreshFooter(context, config);
		/* 2014 04 22 cgp */
		mFooterView.hide(); 
		/* 2014 04 22 cgp */
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setState(KPullFreshHeader.STATE_READY);
			} else {
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
													// more.
				mFooterView.setState(KPullFreshFooter.STATE_READY);
			} else {
				mFooterView.setState(KPullFreshFooter.STATE_NORMAL);
			}
		}
	...
	public void startLoadMore() {
		mPullLoading = true;
		mFooterView.setState(KPullFreshFooter.STATE_LOADING);
		...
		default:
			...
			if (getFirstVisiblePosition() == 0) {
				...
				if (mEnablePullRefresh && mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
					mPullRefreshing = true;
					mHeaderView.setState(KPullFreshHeader.STATE_REFRESHING);
					if (mListViewListener != null) {
						mListViewListener.onRefresh();
					}
				}
